...
protected:
    void SetUp() override {
        targetDevice = "GPU.1";
        {
            auto capabilities = core->get_property(targetDevice, ov::device::capabilities);

            std::cout << "\n=== GPU Device Info (" << targetDevice << ") ===" << std::endl;
            std::cout << "Device: " << core->get_property(targetDevice, ov::device::full_name) << std::endl;
            std::cout << "Architecture: " << core->get_property(targetDevice, ov::device::architecture) << std::endl;
            try {
                std::cout << "Driver version: " << core->get_property(targetDevice, ov::intel_gpu::driver_version) << std::endl;
            } catch (...) {
                std::cout << "Driver version: N/A" << std::endl;
            }
            std::cout << "Execution units: " << core->get_property(targetDevice, ov::intel_gpu::execution_units_count) << std::endl;
            std::cout << "Total memory: " << core->get_property(targetDevice, ov::intel_gpu::device_total_mem_size) / (1024*1024) << " MB" << std::endl;
            auto gops = core->get_property(targetDevice, ov::device::gops);
            for (const auto& [type, val] : gops)
                std::cout << "GOPS [" << type << "]: " << val << std::endl;
...
// Benchmark test for SDPA with f16, Q/K/V shapes: [1, 16, 4096, 64]
class SDPABenchmark : virtual public ov::test::SubgraphBaseStaticTest {
protected:
    void SetUp() override {
        targetDevice = "GPU";
            auto capabilities = core->get_property(targetDevice, ov::device::capabilities);
            ...
            std::cout << "Architecture: " << core->get_property(targetDevice, ov::device::architecture) << std::endl;
        auto inType = ov::element::f16;
        ov::Shape qkvShape{1, 16, 4096, 64};

        auto query = std::make_shared<ov::op::v0::Parameter>(inType, qkvShape);
        auto key = std::make_shared<ov::op::v0::Parameter>(inType, qkvShape);
        auto value = std::make_shared<ov::op::v0::Parameter>(inType, qkvShape);

        auto sdpa = std::make_shared<ov::opset13::ScaledDotProductAttention>(query, key, value, false);
        sdpa->set_friendly_name("sdpa");

        auto output = std::make_shared<ov::op::v0::Result>(sdpa->output(0));
        function = std::make_shared<ov::Model>(ov::OutputVector{output},
                                               ov::ParameterVector{query, key, value},
                                               "sdpa_benchmark_model");
...
class SDPABenchmarkLayerTest : public ov::test::BenchmarkLayerTest<SDPABenchmark> {};

TEST_F(SDPABenchmarkLayerTest, DISABLED_benchmark_f16_16x4096x64) {
    run_benchmark("MLIROp");
}

TEST_F(SDPABenchmark, DISABLED_benchmark_f16_16x4096x64) {
    constexpr int warmup_iters = 100;
    constexpr int bench_iters = 100;

    compile_model();

    // Dump runtime graph nodes
    auto exec_model = compiledModel.get_runtime_model();
    std::cout << "\n=== Runtime model ops ===" << std::endl;
    for (const auto& op : exec_model->get_ordered_ops()) {
        std::string layer_type = "N/A";
        if (op->get_rt_info().count(ov::exec_model_info::LAYER_TYPE))
            layer_type = op->get_rt_info().at(ov::exec_model_info::LAYER_TYPE).as<std::string>();
        std::cout << "name: [" << op->get_friendly_name()
    ...
    for (const auto& param : function->get_parameters()) {
        auto tensor = ov::test::utils::create_and_fill_tensor(
            ...
            param->get_shape(),
            ov::test::utils::InputGenerateData(0, 8, 32, 1));
        infer_request.set_tensor(param, tensor);
    }
    ...
    std::cout << "\n=== SDPA Benchmark (f16, Q/K/V: [1, 16, 4096, 64]) ===" << std::endl;
    std::cout << "Iterations: " << bench_iters << std::endl;
    std::cout << "Total time: " << total_us << " us" << std::endl;
    std::cout << "Average latency: " << total_us / bench_iters << " us" << std::endl;
    std::cout << "Throughput: " << std::fixed << std::setprecision(2)
              << (1000000.0 * bench_iters / total_us) << " infer/sec" << std::endl;
}
